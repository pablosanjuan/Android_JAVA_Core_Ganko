...
public class MyAdapter_manejo extends  BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup parent) {
        ...
        try{
            List<ManejoVO> eventos=new DbManager_manejo(a).getRegistrosByIdBovino(id);

            if(eventos!=null){
                for(int i=0;i<eventos.size();i++){
                    ManejoVO eve=eventos.get(i);

                    tv_registro4.setText(tv_registro4.getText()+eve.getEvento()+"\n");
